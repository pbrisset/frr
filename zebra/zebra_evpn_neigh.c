 ...
 * bridge master is resolved the same way as on install (the ETAG's access BD).
 */
static void zebra_evpn_l3vni_sync_mac_del(struct zebra_neigh *n)
{
	...
	(void)zebra_evpn_l3vni_etag_svi(n->zevpn->vni, n->eth_tag, &acc_bd);
	bond_ifp = if_lookup_by_index_per_ns(zebra_ns_lookup(NS_DEFAULT),
					     n->sync_mac_ifindex);
	if (acc_bd && acc_bd->bridge_zif && acc_bd->bridge_zif->ifp && bond_ifp)
		dplane_local_mac_del(bond_ifp, acc_bd->bridge_zif->ifp,
				     n->eth_tag, &n->emac);
...
static void zebra_evpn_l3vni_remote_neigh_uninstall(struct zebra_neigh *n)
{
	...
	svi_ifp = zebra_evpn_l3vni_etag_svi(n->zevpn->vni, n->eth_tag, NULL);
	if (svi_ifp)
				       ...
				       const esi_t *esi, bool is_router)
{
	...
	if (!n) {
		...
		/* An ETAG change moves the kernel neighbor to another SVI. */
		if (etag_chg) {
			struct interface *old_svi;

			old_svi = zebra_evpn_l3vni_etag_svi(vni, n->eth_tag,
							    NULL);
			if (old_svi)
