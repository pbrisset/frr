...
void bgp_upa_extcom_new(struct in_addr router_id, uint8_t flags,
			struct ecommunity_val *eval)
{
			  ...
			  struct in_addr *router_id_out)
{
	...
	if (router_id_out)
		memcpy(&router_id_out->s_addr,
		       &eval->val[BGP_UPA_EXTCOM_OFF_ROUTER_ID],
