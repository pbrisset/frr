...
def test_show_bgp_aggregate():
    """
    Verify 'show bgp aggregate' command.
    ...
    # Test basic command
    output = r1.vtysh_cmd("show bgp aggregate")
    assert "203.0.113.0/24" in output or "Aggregate" in output, \
        "show bgp aggregate failed"

    # Test JSON output
    output = r1.vtysh_cmd("show bgp aggregate json")
    try:
        parsed = json.loads(output)
        assert isinstance(parsed, dict), "aggregate json didn't return dict"
    except json.JSONDecodeError:
        pytest.fail("show bgp aggregate json returned invalid JSON")

    # Test filtering by prefix
    output = r1.vtysh_cmd("show bgp aggregate 203.0.113.0/24")
    assert "203.0.113.0/24" in output, \
        "show bgp aggregate with prefix filter failed"

    # Verify UPA fields appear in output
    output = r1.vtysh_cmd("show bgp aggregate 203.0.113.0/24")
    assert "UPA" in output or "upa" in output.lower(), \
        "UPA fields not shown in aggregate output"

    # Verify max-routes appears in text output
    assert "max-routes" in output.lower() or "50" in output, \
        "max-routes value not shown in aggregate output"

    # Verify max-routes in JSON output
    output = r1.vtysh_cmd("show bgp aggregate 203.0.113.0/24 json")
    try:
        parsed = json.loads(output)
        # Check for upaMaxRoutes field
        for prefix_data in parsed.values():
            if isinstance(prefix_data, dict):
                if prefix_data.get("upaEnabled"):
                    assert "upaMaxRoutes" in prefix_data, \
                        "upaMaxRoutes field missing from JSON output"
                    assert prefix_data["upaMaxRoutes"] == 50, \
                        f"Expected max-routes=50, got {prefix_data['upaMaxRoutes']}"
                    break
    except (json.JSONDecodeError, KeyError, AssertionError) as e:
        pytest.fail(f"max-routes not properly shown in JSON: {e}")
...
def test_max_routes_dynamic_changes():
    ...
    # Test 5: Verify in show bgp aggregate
    output = r1.vtysh_cmd("show bgp aggregate 198.18.0.0/15 json")
    try:
        parsed = json.loads(output)
        for prefix_data in parsed.values():
